<?PHP
	...
	else if($course_code != NULL AND $course_desc != NULL){
	...
	if($select_statement_valid == 1){
		...
		if($resultSet->num_rows > 0){
			...
			while($rows = $resultSet->fetch_assoc()){
				...
				/*value='$ID_course +'*/
				echo "<br/br/><form action='./courses.html' method='GET'><button type='submit' name='ID_course' id='ID_course' value='$post_string'>Course ID: $ID_course, Course Code: $course_code, Course Description: $course_desc </button></form>";	
			}
	...
	if($course_code != "" ){
		$INSERT = "INSERT INTO t_courses (course_code, course_desc, course_active) VALUES ('$course_code', '$course_desc', '$course_active')";
...
//$UPDATE = "UPDATE t_courses SET course_code ='$course_code', course_desc ='$course_desc' WHERE ID_course='$ID_course'";
			$UPDATE = "UPDATE t_courses SET course_code='$course_code', course_desc='$course_desc', course_active ='$course_active' WHERE ID_course='$ID_course'";

		//$UPDATE = "UPDATE t_courses SET course_code ='$course_code', ";
		//$UPDATE = $UPDATE + "course_desc ='$course_desc', ";
		//$UPDATE = $UPDATE + "course_active ='$course_active' WHERE ID_course='$ID_course'";
	...
	/*Deleting a student*/
		if($ID_course != ""){
			...
			echo "Error in deleting student must include course ID to delete record.";
			echo "<form action='./courses.html' method='get'><input type='submit' value='Go Back to Manage Course'/></form>";
			die();
		}
    }
    else{
